...
class UnitHistProps(PropWidget):
    ...
    def edit_update(self, p_str):
        ...
        chk_val = cf.check_edit_num(nw_val, min_val=1, max_val=n_met, is_int=True)
        if chk_val[1] is None:
            # updates the parameter value
            setattr(self, p_str, chk_val[0])

            # calculates the other dimension value
            p_str_c = 'n_col' if (p_str == 'n_row') else n_row
            p_dim_nw = np.ceil(n_met / chk_val[0])
            setattr(self, p_str_c, p_dim_nw)
