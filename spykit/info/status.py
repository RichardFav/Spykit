...
class StatusInfoTab(InfoWidgetPara):
    ...
    def setup_prop_fields(self):
        ...
        pp_str = [
            # common parameters
            {
                'method': cw.create_para_field('Detection Method', 'combobox', met_list[0], p_list=met_list),
                'noisy_channel_threshold': cw.create_para_field('Noisy Channel Threshold', 'edit', 1.0),
                'chunk_duration_s': cw.create_para_field('Chunk Duration (s)', 'edit', 0.3),
                'num_random_chunks': cw.create_para_field('Random Chunk Count', 'edit', 100),
                'welch_window_ms': cw.create_para_field('Welch Window Size', 'edit', 10.0),
                'highpass_filter_cutoff': cw.create_para_field('Highpass Filter Cutoff', 'edit', 300.0),
                'neighborhood_r2_threshold': cw.create_para_field('Neighbourhood Threshold', 'edit', 0.9),
                'neighborhood_r2_radius_um': cw.create_para_field('Neighbourhood Radius', 'edit', 30.),
            },

            # coherence+psd method parameters
            {
                'psd_hf_threshold': cw.create_para_field('PSD Threshold', 'edit', 0.02),
                'dead_channel_threshold': cw.create_para_field('Dead Channel Threshold', 'edit', -0.5),
                'outside_channel_threshold': cw.create_para_field('Outside Channel Threshold', 'edit', -0.75),
                'outside_channels_location': cw.create_para_field('Outside Channel Location', 'combobox',
                                                                    loc_list[0], p_list=loc_list),
                'n_neighbors': cw.create_para_field('Channel Neighbours', 'edit', 11),
                'nyquist_threshold': cw.create_para_field('Nyquist Threshold', 'edit', 0.8),
                'direction': cw.create_para_field('Depth Direction', 'combobox', dir_list[1], p_list=dir_list),
            },

            # std dev/map method parameters
            {
                'std_mad_threshold': cw.create_para_field('Multiplier Threshold', 'edit', 5.),
            },
        ]
